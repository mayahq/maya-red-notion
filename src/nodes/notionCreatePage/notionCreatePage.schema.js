...
class NotionCreatePage extends Node {
  ...
  static schema = new Schema({
    ...
    fields: {
      ...
      children: new fields.Typed({
        type: "json",
        defaultVal: "{}",
        allowedTypes: ["msg", "flow", "global"],
      }),
      icon: new fields.Typed({
        type: "json",
        defaultVal: "{}",
        allowedTypes: ["msg", "flow", "global"],
      }),
      cover: new fields.Typed({
        type: "json",
        defaultVal: "{}",
        allowedTypes: ["msg", "flow", "global"],
      }),
  ...
  async onMessage(msg, vals) {
    ...
    rows.forEach((row) => {
      ...
      if (vals.parent_type === "database") {
        configBody["parent"]["database_id"] = getDatabaseId(url)
      } else {
        configBody["parent"]["page_id"] = getPageId(url)
      }
      if (vals.children && Object.keys(vals.children).length > 0)
        configBody["children"] = vals.children
      if (vals.icon && Object.keys(vals.icon).length > 0)
        configBody["icon"] = vals.icon
      if (vals.cover && Object.keys(vals.cover).length > 0)
        configBody["cover"] = vals.cover
    ...
    try {
      const result = await Promise.all(
        requests.map((req) => makeRequestWithRefresh(this, req))
      )
